import { JOOXPlatform } from "../../platforms/JOOXPlatform";

export class JOOXService
{

    /**
     * @description JOOX通用搜索处理器 (内部私有)
     */
    private static handleSearch(
        ctx: MainAppContainerActionContext,
        itemsPerPage: number,
        searchFn: (k: string, p: number) => Promise<any>,
        getOnDemandPlay: (platform: JOOXPlatform) => (data: PlatformData) => void
    )
    {
        ...
        if (currentPage <= maxPage)
        {
            ...
            const onDemand = isCurrentInMultiPage ? JOOXPlatform.MOD : getOnDemandPlay(JOOXPlatform);
        ...
        else if (!updating)
        {
            ...
            // 计算页码 (JOOX 30 条/页)
            const page = currentPage !== 1 ? Math.floor(allMediaData.length / 30) + 1 : 1;
            const dataPromise = searchFn(searchKeyword, page);
            const onDemand = getOnDemandPlay(JOOXPlatform);
    ...
    static searchMusic(ctx: MainAppContainerActionContext, itemsPerPage: number)
    {
        this.handleSearch(ctx, itemsPerPage,
            (k, p) => JOOXPlatform.searchSongs(k, p),
            () => JOOXPlatform.MOD
        );
    }
